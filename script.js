let goods = [
  ...
  function showGoods(filteredGoods = goods) {
    let list = document.getElementById("list");
    list.innerHTML = ""; 
    for (let index = 0; index < filteredGoods.length; index++) {
        let li = document.createElement("li");
        li.innerHTML = `<p>${filteredGoods[index].product_name}</p>
                        <p>${filteredGoods[index].product_description}</p>
                         <button class="addButton" id="${filteredGoods[index].id}">Add</button>`;
        list.appendChild(li);
    }
    let buttons = [...document.querySelectorAll('.addButton')];
    buttons.forEach(button => {
        button.addEventListener('click', () => {
            let selectedProduct = goods.find(product => product.id === button.id);
...
function addToLocalStorage(product) {
    ...
    console.log(`${product.product_name} has been added to local storage.`);
}
showGoods()
...
//     buttons.find(button=>button.id===product.id).addEventListener('click', () => {
//         let selectedProduct = goods.find(product=>product.id===button.id)
//         console.log(selectedProduct)
//     });
// })
